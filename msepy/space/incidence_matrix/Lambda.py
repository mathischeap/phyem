...
class MsePyIncidenceMatrixLambda(Frozen):
    ...
    def __call__(self, degree):
        ...
        if key in self._cache:
            ...
            if self._m == 2 and self._n == 2:  # for k == 0 and k == 1.
    ...
    def _m3_n3_k2(self, p):
        """div or d of 2-form"""
        sn = self._space.local_numbering.Lambda._m3_n3_k2(p)
        dn = self._space.local_numbering.Lambda._m3_n3_k3(p)
        E = np.zeros(
            (
                self._space.num_local_dofs.Lambda._m3_n3_k3(p),
                self._space.num_local_dofs.Lambda._m3_n3_k2(p)
            ),
            dtype=int
        )
    ...
    def _m3_n3_k1(self, p):
        """curl or d of 1-form"""
        sn = self._space.local_numbering.Lambda._m3_n3_k1(p)
        dn = self._space.local_numbering.Lambda._m3_n3_k2(p)
        E = np.zeros(
            (
                self._space.num_local_dofs.Lambda._m3_n3_k2(p),
                self._space.num_local_dofs.Lambda._m3_n3_k1(p)
            ),
            dtype=int
        )
    ...
    def _m3_n3_k0(self, p):
        """grad or d of 0-form"""
        sn = self._space.local_numbering.Lambda._m3_n3_k0(p)
        dn = self._space.local_numbering.Lambda._m3_n3_k1(p)
        E = np.zeros(
            (
                self._space.num_local_dofs.Lambda._m3_n3_k1(p),
                self._space.num_local_dofs.Lambda._m3_n3_k0(p)
            ),
            dtype=int
        )
    ...
    def _m2_n2_k0_inner(self, p):
        """grad or d of inner 0-form"""
        sn = self._space.local_numbering.Lambda._m2_n2_k0(p)
        dn = self._space.local_numbering.Lambda._m2_n2_k1_inner(p)
        E = np.zeros(
            (
                self._space.num_local_dofs.Lambda._m2_n2_k1(p),
                self._space.num_local_dofs.Lambda._m2_n2_k0(p)
            ),
            dtype=int
        )
    ...
    def _m2_n2_k1_inner(self, p):
        ...
        sn = self._space.local_numbering.Lambda._m2_n2_k1_inner(p)
        dn = self._space.local_numbering.Lambda._m2_n2_k2(p)
        E = np.zeros(
            (
                self._space.num_local_dofs.Lambda._m2_n2_k2(p),
                self._space.num_local_dofs.Lambda._m2_n2_k1(p)
            ),
            dtype=int
        )
    ...
    def _m2_n2_k0_outer(self, p):
        """curl or d of outer-0-form"""
        sn = self._space.local_numbering.Lambda._m2_n2_k0(p)
        dn = self._space.local_numbering.Lambda._m2_n2_k1_outer(p)
        E = np.zeros(
            (
                self._space.num_local_dofs.Lambda._m2_n2_k1(p),
                self._space.num_local_dofs.Lambda._m2_n2_k0(p)
    ...
    def _m2_n2_k1_outer(self, p):
        ...
        sn = self._space.local_numbering.Lambda._m2_n2_k1_outer(p)
        dn = self._space.local_numbering.Lambda._m2_n2_k2(p)
        E = np.zeros(
            (
                self._space.num_local_dofs.Lambda._m2_n2_k2(p),
    ...
    def _m1_n1_k0(self, p):
        ...
        E = np.zeros(
            (
                self._space.num_local_dofs.Lambda._m1_n1_k1(p),
                self._space.num_local_dofs.Lambda._m1_n1_k0(p)
            ),
            dtype=int
        )
        for i in range(p[0]):

...
class MsePySpaceReduceBundle(Frozen):
    ...
    def _m2_n2_k0(self, cf, t, degree):
        ...
        nodes = self._space[degree].nodes
        xi, et = np.meshgrid(*nodes[0], indexing='ij')
        xi = xi.ravel('F')
        et = et.ravel('F')
        x0, y0 = self._mesh.ct.mapping(xi, et)

        xi, et = np.meshgrid(*nodes[1], indexing='ij')
        xi = xi.ravel('F')
        et = et.ravel('F')
        x1, y1 = self._mesh.ct.mapping(xi, et)

        local_cochain_x = []
        local_cochain_y = []
        for ri in cf.field:
            vector0 = cf.field[ri]._v0_  # the scalar evaluated at time `t`.
            vector1 = cf.field[ri]._v1_  # the scalar evaluated at time `t`.
            start, end = self._mesh.elements._elements_in_region(ri)
            x_region = x0[..., start:end]
            y_region = y0[..., start:end]
            x_local_cochain_region = vector0(t, x_region, y_region)
            x_region = x1[..., start:end]
            y_region = y1[..., start:end]
            y_local_cochain_region = vector1(t, x_region, y_region)

            local_cochain_x.append(x_local_cochain_region)
            local_cochain_y.append(y_local_cochain_region)

        local_cochain_x = np.concatenate(local_cochain_x, axis=1).T
        local_cochain_y = np.concatenate(local_cochain_y, axis=1).T

        local_cochain = np.hstack((local_cochain_x, local_cochain_y))

        return local_cochain

    def _n2_k1_preparation(self, d_, degree, quad_degree, component):
        key = d_ + str(degree) + str(quad_degree) + str(component)
        if key in self._cache221:
            data = self._cache221[key]
        else:
            nodes = self._space[degree].nodes[component]
            p = self._space[degree].p[component]
            qp = quad_degree
    ...
    def _m2_n2_k1_inner(self, cf, t, degree, quad_degree=None):
        ...
        for i in range(2):
            ...
            local_cochains.append(
                cochain_local
            )

        return np.hstack(local_cochains)
    ...
    def _preparation_m2n2k2(self, degree, quad_degree, component):
        """"""
        key = str(degree) + str(quad_degree) + str(component)
        if key in self._cache222:
            ...
            nodes = self._space[degree].nodes[component]
            num_basis = self._space.num_local_dof_components(degree)[component]
    ...
    def _m3_n3_k2(self, cf, t, degree, quad_degree=None):
        ...
        for _I_ in range(3):
            ...
            if key in self._cache332:
                ...
                num_basis_components = self._space[degree].num_local_dof_components[_I_]
                p = self._space[degree].p[_I_]
                nodes = self._space[degree].nodes[_I_]
    ...
    def _m3_n3_k1(self, cf, t, degree, quad_degree=None):
        ...
        for _I_ in range(3):
            ...
            ux, vx, wx = cf[t](*coo_dx, axis=-1)[_I_]
            uy, vy, wy = cf[t](*coo_dy, axis=-1)[_I_]
            uz, vz, wz = cf[t](*coo_dz, axis=-1)[_I_]
    ...
    def _m3n3k1_preparation(self, d_, degree, quad_degree, component):
        """
        Parameters
        ----------
        d_ : str, optional
    ...
    def _m3_n3_k0(self, cf, t, degree):
        ...
        for _I_ in range(3):
            ...
            local_cochain = []
            for ri in cf.field:
                scalar = cf.field[ri]._vs_[_I_]  # the scalar evaluated at time `t`.
                start, end = self._mesh.elements._elements_in_region(ri)
                x_region = x[..., start:end]
                y_region = y[..., start:end]
                z_region = z[..., start:end]
                local_cochain_region = scalar(t, x_region, y_region, z_region)

                local_cochain.append(local_cochain_region)

            local_cochain = np.concatenate(local_cochain, axis=1).T

            local_cochains.append(local_cochain)

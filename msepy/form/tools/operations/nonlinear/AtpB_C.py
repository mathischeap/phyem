            ...
            if cache_index in _data_cache:
                ...
                if self._type == 'bundle':
                    ...
                    if self._mesh.n == 2:
                        ...
                        # B = [b0, b1]
                        # C = ([c00, c01], [c10, c11])
                        # A otimes B = ([a0 * b0, a0 * b1], [a1 * b0, a1 * b1])
                        # (A otimes B).dot(C) = (
                        #     [a0 * b0 * c00, a0 * b1 * c01],
                        #     [a1 * b0 * c10, a1 * b1 * c10]
                        # )
                        ...
                        o00 = np.einsum(
                            'li, lj, lk, l -> ijk', a0, b0, c00, metric, optimize='optimal'
                        )
                        o01 = np.einsum(
                            'li, lj, lk, l -> ijk', a0, b1, c01, metric, optimize='optimal'
                        )
                        o10 = np.einsum(
                            'li, lj, lk, l -> ijk', a1, b0, c10, metric, optimize='optimal'
                        )
                        o11 = np.einsum(
                            'li, lj, lk, l -> ijk', a1, b1, c11, metric, optimize='optimal'
                        )

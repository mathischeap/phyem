...
class MseHttDynamicNonLinearSystem(Frozen):
    ...
    def __init__(self, wf_mp_nls, base):
        ...
        self._dls = MseHttDynamicLinearSystem(wf_mp_nls._mp_ls, base)  # the dynamic linear system
        self._tfs = None
        self._unknowns = None
        ...
        self._config = MseHtt_Dynamic_NonLinear_System_Config(self)
        _ = self._dls.config
        self._freeze()
    ...
    def _parse_nonlinear_terms(self):
        ...
        for i, terms in enumerate(all_terms):
            ...
            for sign, term in zip(signs, terms):
                ...
                parsed_term, text, time_indicator = nonlinear_operator_parser(term, self.base)
                assert parsed_term.__class__ in (MseHttDynamicLocalNonlinearOperator,), \
                    f"msepy nonlinear term must be represented by one of ({MseHttDynamicLocalNonlinearOperator, })"
    ...
    def __call__(self, *args, **kwargs):
        ...
        bc = self.bc  # We have to send it to the static local nonlinear system
        ...
        static_nls = MseHttStaticNonLinearSystem(
            # the linear part:
            static_A, static_x, static_b,

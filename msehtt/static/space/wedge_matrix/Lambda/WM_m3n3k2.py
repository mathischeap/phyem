...
def wedge_matrix_Lambda__m3n3k2_w_m3n3k1(tpm, degree0, degree1):
    r""""""
    key = tpm.__repr__() + _degree_str_maker(degree0) + '<D>' + _degree_str_maker(degree1)
    if key in _cache_wm332_331_:
        return _cache_wm332_331_[key]
    ...
    for e in tpm.composition:
        ...
        if etype in (
            ...
            W[e], cache_key_dict[e] = ___wm332_331_orthogonal_hexahedron___(element, degree0, degree1)

        else:
            raise NotImplementedError(f"{__name__} not implemented for etype={etype}")

    _cache_wm332_331_[key] = W, cache_key_dict
...
def ___wm332_331_orthogonal_hexahedron___(element, degree0, degree1):
    r""""""
    key = _degree_str_maker(degree0) + ' w ' + _degree_str_maker(degree1)
    if key in _cache_332_331_:
        W, cache_key = _cache_332_331_[key]
    else:
        ...
        _, bf0 = element.bf('m3n3k2', degree0, *quad_nodes)
        _, bf1 = element.bf('m3n3k1', degree1, *quad_nodes)
        W00 = np.einsum(
            'm, im, jm -> ij',
            quad_weights,
            bf0[0], bf1[0],
        ...
        W = csr_matrix(W)
        cache_key = key
        _cache_332_331_[key] = W, cache_key

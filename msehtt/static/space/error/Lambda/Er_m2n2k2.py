...
def error__m2n2k2(tpm, cf, cochain, degree, error_type):
    """"""
...
def _er222_msepy_quadrilateral_(element, cf, local_cochain, degree, error_type):
    """"""
    p = element.degree_parser(degree)[0]
    p = (p[0] + 2, p[1] + 2)
    nodes, weights = quadrature(p, 'Gauss').quad
    x, y, u = ___rc222_msepy_quadrilateral___(element, degree, local_cochain, *nodes, ravel=False)
    exact_u = cf(x, y)[0]

...
def error__m3n3k3(tpm, cf, cochain, degree, error_type):
    """"""
    ...
    for e in elements:
        ...
        if etype in (
            ...
            element_error = _er333_msepy_quadrilateral_(
...
from phyem.msehtt.static.space.reconstruct.Lambda.Rc_m3n3k3 import ___rc333_msepy_quadrilateral___


def _er333_msepy_quadrilateral_(element, cf, local_cochain, degree, error_type):
    """"""
    p = element.degree_parser(degree)[0]
    p = (p[0] + 2, p[1] + 2, p[2] + 2)
    nodes, weights = quadrature(p, 'Gauss').quad
    x, y, z, u = ___rc333_msepy_quadrilateral___(element, degree, local_cochain, *nodes, ravel=False)
    exact_u = cf(x, y, z)[0]

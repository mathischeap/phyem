...
def mass_Lambda__m3n3k3(tpm, degree, cochain):
    """"""
    ...
    for e in elements:
        ...
        if etype in (
            ...
            element_mass = _mass333_msepy_quadrilateral_(
...
def _mass333_msepy_quadrilateral_(element, local_cochain, degree):
    """"""
    p = element.degree_parser(degree)[0]
    p = (p[0] + 2, p[1] + 2, p[2] + 2)
    nodes, weights = quadrature(p, 'Gauss').quad
    _, _, _, u = ___rc333_msepy_quadrilateral___(element, degree, local_cochain, *nodes, ravel=False)

...
class AB_ip_dC(MseHttTrilinearBase):
    ...
    def __init__(self, A, B, C):
        ...
        cache_key = ' <AB_ip_dC> '.join(cache_key)
        self._melt()
    ...
    def clean_cache(cls):
        r""""""
        keys = list(___msehtt_static_cache_AB_ip_dC_3d_data___.keys())
        for key in keys:
            del ___msehtt_static_cache_AB_ip_dC_3d_data___[key]
    ...
    def _make_3d_data(self):
        ...
        elif self._cache_key in ___msehtt_static_cache_AB_ip_dC_3d_data___:
            self._3d_data = ___msehtt_static_cache_AB_ip_dC_3d_data___[self._cache_key]
        # ------- make the data -------------------------------------------------------------
        else:
            _3d_data = self._generate_data_()
            self._3d_data = _3d_data
            ___msehtt_static_cache_AB_ip_dC_3d_data___[self._cache_key] = _3d_data
    ...
    def _generate_data_(self):
        ...
        for e in elements:
            ...
            if isinstance(cache_key, str) and cache_key in _cache_:
                ...
                # So, we only need to consider it like `int_{A B C}`, i.e. integral of (A multi B multi C).

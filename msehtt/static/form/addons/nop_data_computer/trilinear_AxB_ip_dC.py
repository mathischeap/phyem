...
_cache_AxB_ip_dC_3d_data_ = {}


class AxB_ip_dC(MseHttTrilinearBase):
    r"""(AxB, d(C))"""
    ...
    def clean_cache(cls):
        r""""""
        keys = list(_cache_AxB_ip_dC_3d_data_.keys())
        for key in keys:
            del _cache_AxB_ip_dC_3d_data_[key]
    ...
    def _make_3d_data(self):
        ...
        elif self._cache_key in _cache_AxB_ip_dC_3d_data_:
            self._3d_data = _cache_AxB_ip_dC_3d_data_[self._cache_key]
        # ------- make the data -------------------------------------------------------------
        else:
            _3d_data = self._generate_data_()
            self._3d_data = _3d_data
            _cache_AxB_ip_dC_3d_data_[self._cache_key] = _3d_data

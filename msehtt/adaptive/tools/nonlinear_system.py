...
class MseHtt_Adaptive_NonLinear_System(Frozen):
    """"""

    def __init__(self, abs_nls):
        """"""
        assert abs_nls.__class__ is MatrixProxyNoneLinearSystem, f"I need a {MatrixProxyNoneLinearSystem}!"
        self._abs_nls = abs_nls
        self._msehtt_adaptive_ = None
        self._config = _MSEHTT_ADAPTIVE_NON_LINEAR_SYSTEM_CONFIG_(self)
    ...
    def apply(self):
        ...
        static_dynamic_nLS = MseHttDynamicNonLinearSystem(
            self._abs_nls, BASE
        ).apply()
        ...
        for CONFIG in configurations:
            ...
            static_dynamic_nLS.config(*ARGS, **KWARGS)

        self._generation_cache_ = generations
        self._dynamic_cache_ = static_dynamic_nLS

        return static_dynamic_nLS
...
class _MSEHTT_ADAPTIVE_NON_LINEAR_SYSTEM_CONFIG_(Frozen):
    r""""""

    def __init__(self, ada_nls):
        r""""""
        self._ada_nls = ada_nls

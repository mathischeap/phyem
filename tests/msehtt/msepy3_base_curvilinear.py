...
mpiexec -n 4 python tests/msehtt/msepy3_base_curvilinear.py
"""
...
assert all([_ < 0.005 for _ in [error0, error1, error2, error3]]), f"reduce error!"
...
norm = of1[0].norm()
np.testing.assert_almost_equal(norm, NORM_f, decimal=3)
norm = of2[0].norm()
np.testing.assert_almost_equal(norm, NORM_f, decimal=2)
norm = of3[0].norm()
np.testing.assert_almost_equal(norm, NORM_w, decimal=3)

# -*- coding: utf-8 -*-
r"""
mpiexec -n 4 python tests/msehtt/flow_around_cylinder_0dvdx_outlet.py
"""
...
h0 = Out0.make_form(r'\tilde{h0}', 'helper0')
h1 = Out1.make_form(r'\tilde{h1}', 'helper1')
...
pde.bc.partition(r"\Gamma_{\parallel}", r"\Gamma_\omega")
pde.bc.define_bc(
    {
        r"\Gamma_\omega": ph.trace(w),       # essential BC: norm velocity.
    }
)
...
boundary_w = msehtt.base['meshes'][r"\partial\mathfrak{M}\left(\Gamma_\omega\right)"]
boundary_para = msehtt.base['meshes'][r"\partial\mathfrak{M}\left(\Gamma_{\parallel}\right)"]
...
results_dir = './__phcache__/msehtt_flow_around_cylinder_dvdx/'

# msehtt.info()

msehtt_nls = obj['nls'].apply()
msehtt_nls.config(('essential bc', 1), boundary_u, bc_velocity, root_form=u)  # essential bc
msehtt_nls.config(['natural bc', 1], boundary_p, u.numeric.tsp.L2_energy(), root_form=P)  # natural bc
...
msehtt_nls.config(('essential bc', 1), boundary_w, tsp, root_form=w)  # essential bc

for step in range(1, total_steps+1):
    t_m1 = ts['k-1'](k=step)()
    p0x, p0y = u[t_m1].project.to('m2n2k0')
    p0[t_m1].cochain = p0x
    p1[t_m1].cochain = p0[t_m1].cochain.coboundary()
    ...
    # w[None].visualize.matplot()

    if step % 100 == 0:
        ph.vtk(results_dir + f'step1_{step}', u[None], w[None], P[None])
    else:
        pass

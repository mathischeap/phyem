# -*- coding: utf-8 -*-
r"""
mpiexec -n 4 python tests/msehtt/raw/normal_dipole_collision_t.py
...
tgm = msehtt.tgm()
# msehtt.config(tgm)('crazy', element_layout=20, c=0, bounds=[[-1., 1], [-1, 1]], periodic=False)
msehtt.config(tgm)('chaotic', element_layout=10, bounds=[[-1, 1], [-1, 1]], periodic=False)
# tgm.visualize()
...
for step in range(1, total_steps+1):
    ...
    w[None].visualize.matplot(
        saveto=results_dir + f'w{step}.png', levels=np.linspace(-300, 300, 50)
    )
    # else:
    #     pass

...
mpiexec -n 4 python tests/msehtt/customized_nodes/cus_nodes_base3.py
"""
import numpy as np
import phyem as ph
...
# print(of0[None].error())
# print(of1[None].error())
# print(of2[None].error())
# print(of3[None].error())

assert of0[None].error() < 6e-5
assert of1[None].error() < 6e-4
assert of2[None].error() < 8e-4
assert of3[None].error() < 6e-4
...
norm = of1[0].norm()
np.testing.assert_almost_equal(norm, NORM_f, decimal=4)
norm = of2[0].norm()
np.testing.assert_almost_equal(norm, NORM_f, decimal=4)
norm = of3[0].norm()
np.testing.assert_almost_equal(norm, NORM_w, decimal=4)

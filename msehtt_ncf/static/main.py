...
base = {
    ...
    'the_great_mesh': None,   # all forms/meshes will point to this great mesh.
...
import msehtt_ncf.static.implementation_array_parser as PARSER
PARSER._setting_['base'] = base
base['PARSER'] = PARSER

import msehtt_ncf.static.implementation_nop_parser as NOC_PARSER
NOC_PARSER._setting_['base'] = base
base['NOC-PARSER'] = NOC_PARSER

from msehtt_ncf.static.manifold.main import MseHtt_NCF_Static_Manifold
from msehtt_ncf.static.mesh.partial.main import MseHtt_NCF_Static_PartialMesh
from msehtt_ncf.static.mesh.great.main import MseHtt_NCF_Static_GreatMesh
from msehtt_ncf.static.space.main import MseHtt_NCF_Static_Space
from msehtt_ncf.static.form.main import MseHtt_NCF_Static_Form
...
def _parse_manifolds(abstract_manifolds):
    ...
    for sym in abstract_manifolds:
        manifold = MseHtt_NCF_Static_Manifold(abstract_manifolds[sym])
    ...
    for ab_msh_sym_repr in abstract_spaces:
        ...
        for ab_sp_sym_repr in ab_sps:
            ...
            if ab_sp.orientation == 'unknown':
                ...
                space = MseHtt_NCF_Static_Space(ab_sp)
...
def info(*others_2b_printed):
    ...
    print(f'==msehtt-ncf== [{count}] {MyTimer.current_time()} -after- %.2f(s)'
...
def array(indicator, *args, **kwargs):
    ...
    if indicator == 'mass matrix':
        PARSER = base['PARSER']
        if (len(args) == 1) and (kwargs == {}):  # in this case, we must receive a msehtt-ncf static form.
            form = args[0]
            assert form.__class__ is MseHtt_NCF_Static_Form, f"we must have received a msehtt-ncf static form."

...
from legacy.generic.py.matrix.localize.bmat import bmat
from legacy.generic.py.vector.localize.concatenate import concatenate

from legacy.generic.py.linear_system.localize.static.customize import _Customize

from legacy.generic.py.linear_system.globalize.static import Globalize_Static_Linear_System


class Localize_Static_Linear_System(Frozen):
    """"""
    ...
    def _parse_gathering_matrices(self, A, x, b):
        ...
        for j in range(col_shape):
            x_j = x[j]
            assert x_j.__class__ is Localize_Static_Vector_Cochain, f"x[{j}] is {x_j.__class__}, wrong"
            gm_j = x_j._gm
        ...
        for i in range(row_shape):
            ...
            if b_i is None:
                ...
                assert b_i.__class__ in (Localize_Static_Vector, Localize_Static_Vector_Cochain), \
    ...
    def assemble(self, cache=None):
        ...
        ALS = Globalize_Static_Linear_System(A, b)
        return ALS
    ...
    def pr(self):
        ...
        if _setting['pr_cache']:
            _pr_cache(fig, filename='msehy_staticLocalLinearSystem')
        else:
...
class _Xxx(Frozen):
    ...
    def __init__(self, ls, x):
        ...
        for c in range(ls.shape[1]):
            assert x[c].__class__ is Localize_Static_Vector_Cochain, f"x[{c}] is not a static cochain vector."
            time_slots.append(
                x[c]._t
            )

...
from legacy.generic.py.vector.localize.dynamic import Localize_Dynamic_Vector, Localize_Dynamic_Vector_Cochain
...
from legacy.generic.py.linear_system.localize.dynamic.array_parser import py_array_parser

from legacy.generic.py.linear_system.localize.static.main import Localize_Static_Linear_System
...
class Dynamic_Linear_System(Frozen):
    ...
    def _set_bc(self, bc, base):
        """set boundary condition."""
        if bc is None:
            self._bc = None
        else:
            self._bc = Dynamic_Linear_System_BC(
                self, bc, base
            )
    ...
    def apply(self):
        """"""
        self._set_bc(self._mp_ls._bc, self._base)
    ...
    def __call__(self, *args, **kwargs):
        ...
        return Localize_Static_Linear_System(
            static_A, static_x, static_b,
    ...
    def _get_raw_Axb(self, *args, **kwargs):
        ...
        for j in range(num_cols):
            ...
            assert static_x_j.__class__ is Localize_Static_Vector_Cochain, \
        ...
        predefined_sls = Localize_Static_Linear_System(static_A, static_x, static_b)
        ...
        for i in range(num_rows):
            for j in range(num_cols):
                Aij = static_A[i][j]
                if Aij is None:
                    static_A[i][j] = Localize_Static_Matrix(
            ...
            if static_b[i] is None:
                static_b[i] = Localize_Static_Vector(0, row_gms[i])
            else:
                pass
    ...
    def pr(self, figsize=(10, 4)):
        ...
        if _setting['pr_cache']:
            _pr_cache(fig, filename='msehy_DynamicLinearSystem')
        else:
            plt.tight_layout()
...
class DynamicBlockEntry(Frozen):
    ...
    def __call__(self, *args, **kwargs):
        ...
        assert static.__class__ in (
            Localize_Static_Matrix,
            Localize_Static_Vector_Cochain,
            Localize_Static_Vector,
...
class DynamicTerm(Frozen):
    ...
    def __init__(self, dls, sign, factor, components):
        ...
        for comp_lin_repr in components:
            ...
            Mat, sym_repr, time_indicator = py_array_parser(dls, comp_lin_repr)

            if Mat.__class__ is Localize_Static_Matrix:
                Mat = Localize_Dynamic_Matrix(Mat)
            else:
                pass

            assert Mat.__class__ in (
                Localize_Dynamic_Matrix,
                Localize_Dynamic_Vector_Cochain,
                Localize_Dynamic_Vector,

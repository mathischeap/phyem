...
from msehy.tools.vector.dynamic import IrregularDynamicLocalVector
...
class IrregularBase3Entries(Frozen):
    """"""
    def __init__(self):
        self._A = None
        self._B = None
    ...
    def __call__(self, dimensions, *args, **kwargs):
        ...
        if dimensions == 1:
            ...
            return (
                IrregularDynamicLocalVector(_vector_data_caller),
                _vector_data_caller._time_caller,
                _vector_data_caller._generation_caller,
            )
            ...
            if (self._A is not self._B) and (self._B is not self._C):
                return IrregularDynamicLocalNonlinearOperator(
                    self._3d_data,
...
class _OneDimVector(Frozen):
    ...
    def __call__(self, *args, **kwargs):
        """Must return a irregular static local vector"""
        cochains = dict()
        times = self._time_caller(*args, **kwargs)
        g = self._generation_caller(*args, **kwargs)
        assert g[0] == g[1] and g[0] == g[2], f'given forms must be at same generation.'
        g = g[0]
        for f, i, time in zip(self._gfs, self._gis, times):
            cochains[i] = f[(time, g)].cochain.local

        operands = f'ijk, {self._gis[0]}, {self._gis[1]} -> {self._ti}'

        _3d_data = self._3d_data(g)

        c0 = cochains[self._gis[0]]
        c1 = cochains[self._gis[1]]

        local_vector_dict = dict()
        for i in _3d_data:  # all local elements
            data_i = _3d_data[i]

            local_vector_dict[i] = np.einsum(
                    operands,
                    data_i, c0[i], c1[i],
                    optimize='optimal',
            )

        return IrregularStaticLocalVector(
            local_vector_dict, self._tf.cochain.gathering_matrix(g)
        )
    ...
    def _generation_caller(self, *args, g=None, **kwargs):
        ...
        for f in self._gfs:
            generations.append(
                f.cochain._generation_caller(g=g)
            )
        generations.append(
            self._tf.cochain._generation_caller(g=g)
        )
        return generations
...
def _2d_matrix_representation(ABC, _3d_data, row_form, col_form):
    ...
    return IrregularDynamicLocalMatrix(caller)
...
class _D2MatrixCallerRowCol(Frozen):
    ...
    def __call__(self, *args, g=None, **kwargs):
        """"""
        gm_row = self._ABC[self._row].cochain.gathering_matrix(g)
        gm_col = self._ABC[self._col].cochain.gathering_matrix(g)
        given_form = self._ABC[self._given_index]
        given_form_cochain = given_form.cochain._callable_cochain(*args, g=g, **kwargs)
        ...
        return IrregularStaticLocalMatrix(_2d_matrix_caller, gm_row, gm_col, cache_key='unique')

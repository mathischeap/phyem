...
class MseHyPy2SpaceReduceLambda(Frozen):
            ...
            for index in local_cochain:
                ...
                for j, _ in enumerate(gm):
                    if cochain_dict[_] is None:
                        ...
                        assert np.isclose(cochain_dict[_], cochain[j], atol=1e-3), \
    ...
    def _local_k1_inner_evolve(self, func, generation, degree, fc_range):
        """"""
        quad_degree = [p + 2 for p in self._space[degree].p]
        representative = self._mesh[generation]
        ...
        xi, et, edge_size_d, quad_weights = self._k1_preparation('x', degree, quad_degree)
        xy = representative.ct.mapping(xi, et, fc_range=fc_range)
        J = representative.ct.Jacobian_matrix(xi, et, fc_range=fc_range)
        cochain_local_dx = dict()

        for e in J:
            x, y = xy[e]
            u, v = func[0](x, y), func[1](x, y)
            Je = J[e]
            J00, J01 = Je[0]
            J10, J11 = Je[1]
        ...
        q_pp = self._k1_preparation('y', degree, quad_degree)
        xi_q, et_q, edge_size_d_q, quad_weights_q = q_pp
        xy_q = representative.ct.mapping(xi_q, et_q, fc_range=fc_range)
        J_q = representative.ct.Jacobian_matrix(xi, et_q, fc_range=fc_range)

        t_pp = self._k1_preparation('y', degree, quad_degree, triangle_y=True)
        xi_t, et_t, edge_size_d_t, quad_weights_t = t_pp
        xy_t = representative.ct.mapping(xi_t, et_t, fc_range=fc_range)
        J_t = representative.ct.Jacobian_matrix(xi_t, et_t, fc_range=fc_range)
    ...
    def _local_k1_outer_evolve(self, func, generation, degree, fc_range):
        """"""
        quad_degree = [p + 2 for p in self._space[degree].p]
    ...
    def _local_k2(self, func, generation, degree, fc_range):
        quad_degree = [p + 2 for p in self._space[degree].p]
        xi, et, volume, quad_weights = self._k2_preparation(degree, quad_degree)

        representative = self._mesh[generation]
        xy = representative.ct.mapping(xi, et, fc_range=fc_range)
        J = representative.ct.Jacobian(xi, et, fc_range=fc_range)
        cochain_local = dict()
        for e in xy:
            x, y = xy[e]
            f = func(x, y)

...
def _parse(obj):
    ...
    if obj.__class__ is MatrixProxyLinearSystem:
        ...
        dynamic = IrregularDynamicLinearSystem(obj, base)
        return dynamic
    elif obj.__class__ is MatrixProxyNoneLinearSystem:
        from msehy.tools.nonlinear_system.dynamic.main import IrregularDynamicNonLinearSystem
        dynamic = IrregularDynamicNonLinearSystem(obj, base)
        return dynamic
...
def info(*others_2b_printed):
    ...
    for form_sym in forms:
        form = forms[form_sym]
        if form._is_base():
            newest_time, newest_generation = form.cochain.newest
            if newest_time is not None:
                print('{:>20} @ {:<15} >>> mesh[{}]'.format(
                    form.abstract._pure_lin_repr, newest_time, newest_generation
                ))
        else:
            pass

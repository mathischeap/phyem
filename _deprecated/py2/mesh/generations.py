...
class _MesHyPy2MeshGenerations(Frozen):
    ...
    def __getitem__(self, generation):
        ...
        if generation in self._pool:
            pass
        else:
            all_cached_generations = tuple(self._pool.keys())
            max_generation = max(all_cached_generations)
            if generation > max_generation:
                raise Exception(f"generation #{generation} is ahead of the current generation: {max_generation}.")
            else:
                raise Exception(f"generation #{generation} is among the cached generations={all_cached_generations}.")

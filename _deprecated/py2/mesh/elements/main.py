...
class MseHyPy2MeshElements(Frozen):
    ...
    def metric_signature(self, index):
        ms = self[index].metric_signature
        if isinstance(ms, int) or ms is None:
            return 'unique'
        else:
            return ms

    def fc_type_signature(self, index):
        """Used for cache key of for example incidence matrix which is only dependent on the type
        of the cell, `q` or `t`.
        """

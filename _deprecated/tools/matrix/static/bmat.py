...
def bmat(A_2d_list):
    ...
    chain_row_gm = IrregularGatheringMatrix(row_gms)
    chain_col_gm = IrregularGatheringMatrix(col_gms)

    # only adjustments take effect. Customization will be skipped.
    M = _IrregularStaticLocalMatrixBmat(A_2d_list, (row_shape, col_shape))

    return IrregularStaticLocalMatrix(M, chain_row_gm, chain_col_gm, M.cache_key)


class _IrregularStaticLocalMatrixBmat(Frozen):
    """"""

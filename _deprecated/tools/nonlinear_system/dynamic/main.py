...
from msehy.tools.nonlinear_system.dynamic.nonlinear_operator_parser import (
    msehy_nonlinear_operator_parser as __array_parser___)

from msehy.tools.linear_system.dynamic.main import IrregularDynamicLinearSystem
from msehy.tools.nonlinear_system.static.local import IrregularStaticLocalNonLinearSystem
from msehy.tools.nonlinear_operator.dynamic import IrregularDynamicLocalNonlinearOperator

from msehy.py2.main import base as base_py2

from src.config import get_embedding_space_dim
_space_dim = get_embedding_space_dim()
if _space_dim == 2:
    base = base_py2
else:
...
class IrregularDynamicNonLinearSystem(Frozen):
    ...
    def __init__(self, mp_nls, msepy_base):
        ...
        self._dls = IrregularDynamicLinearSystem(mp_nls._mp_ls, msepy_base)  # the dynamic linear system
        self._tfs = None
        self._unknowns = None
        ...
        self._nonlinear_generation_indicators = dict()
    ...
    def _parse_nonlinear_terms(self):
        ...
        for i, terms in enumerate(all_terms):
            ...
            for sign, term in zip(signs, terms):
                ...
                parsed_term, text, time_indicator, generation_indicator = __array_parser___(term)
                assert parsed_term.__class__ in (IrregularDynamicLocalNonlinearOperator,), \
                    f"msepy nonlinear term must be represented by one of ({IrregularDynamicLocalNonlinearOperator, })"
                assert callable(generation_indicator), f"generation_indicator must be callable"

                parsed_terms[i].append(parsed_term)
                parsed_signs[i].append(sign)
                texts[i].append(text)
                time_indicators[i].append(time_indicator)
                generation_indicators[i].append(generation_indicator)
        ...
        self._nonlinear_generation_indicators = generation_indicators
    ...
    def __call__(self, *args, **kwargs):
        ...
        parsed_times = dict()
        parsed_generations = dict()
        parsed_texts = dict()

        for i in self._nonlinear_terms:
            parsed_linear_terms[i] = list()
            parsed_times[i] = list()
            parsed_generations[i] = list()
            parsed_texts[i] = list()
            for j in range(len(self._nonlinear_terms[i])):
                the_nonlinear_term, times, generations, text = self._nonlinear_term_call(i, j, *args, **kwargs)
        ...
        for uk in self.unknowns:
            ...
            generation = uk.cochain._generation_caller(*args, **kwargs)
            unknowns.append(
                uk[(ati_time, generation)]
            )

        # make the static nonlinear system ...
        static_nls = IrregularStaticLocalNonLinearSystem(
            # the linear part:
            static_A, static_x, static_b,
            ...
            nonlinear_generation_indicators=parsed_generations,
            ...
            for atf in abstract_tfs:
                for mf in forms:
                    ...
                    if m_f.abstract is atf:
                        ...
                        pass
            assert len(self._tfs) == self.shape[0], \
                f"test forms too many or too less. {self._tfs}, {self.shape}"
        return self._tfs
    ...
    def _nonlinear_term_call(self, i, j, *args, **kwargs):
        ...
        generations = self._nonlinear_generation_indicators[i][j](*args, **kwargs)

        text = factor._sym_repr + self._nonlinear_texts[i][j]  # add factor sym to the term text.
        return final_term, times, generations, text

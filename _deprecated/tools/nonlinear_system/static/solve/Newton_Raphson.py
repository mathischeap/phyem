...
from msepy.tools.nonlinear_system.static.solve.Newton_Raphson import _nLS_stop_criterion


class IrregularNonlinearSystemNewtonRaphsonSolve(Frozen):
    """"""
    ...
    def x0(self, _x0):
        """"""
        if all([hasattr(_, '_is_discrete_form') and _._is_discrete_form() for _ in _x0]):
            # providing all MsePyRootForm
            cochain = list()

            for f in _x0:
                newest_time, newest_generation = f.cochain.newest  # use the newest cochains.
                gm = f.cochain.gathering_matrix(newest_generation)

                if newest_time is None:  # no newest cochain at all.
                    # then use a cochain of only 0
                    local_cochain = dict()
                    for i in gm:
                        local_cochain[i] = np.zeros_like(gm[i])

                else:
                    local_cochain = f.cochain[(newest_time, newest_generation)].local

                cochain.append(local_cochain)

            self._x0 = cochain   # a list of (dict of 1d-array); the local cochains.
    ...
    def __call__(
        ...
        assert isinstance(x0, list) and all([isinstance(_, dict) for _ in x0]), \
        ...
        while 1:
            ...
            for i in range(S0):
                ...
                for j in range(S1):
                    ...
                    if linear_term is not None:
                        assert linear_term.__class__ is IrregularStaticLocalMatrix, \
                            f"A linear term [{i}][{j}] must be a {IrregularStaticLocalMatrix},"
                    ...
                    if NTi is None:
                        ...
                        for term, sign in zip(NTi, NSi):
                            ...
                            if unknown_form in term.correspondence:
                                ...
                                assert contribution_2_Aij.__class__ is IrregularStaticLocalMatrix, \
                                    (f"contribution of nonlinear term to linear system must be a "
                                     f"{IrregularStaticLocalMatrix},")
            ...
            for uk in self._nls.unknowns:
                x.append(uk._f.cochain.static_vec(uk._t, self._nls._g))

            ls = IrregularStaticLocalLinearSystem(LHS, x, f)
            ...
            for customization in self._nls.customize._customizations:
                ...
                A = ls.A._mA  # local matrix A for cells
                f = ls.b._vb  # local vector b for cells
                # ------------------------------------------------------------ 3
            ...
            for dx_i in dx:
                beta += sum(
                    [np.sum(dx_i[index]**2) for index in dx_i]
                )
        ...
        message += f"<nonlinear_solver>" \
                   f" = [IrregularNewtonRaphson: {A_shape}]" \

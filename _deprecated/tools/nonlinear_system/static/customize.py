...
class IrregularStaticNonlinearSystemCustomize(Frozen):
    """"""
    ...
    def set_x0_from_local_dofs(self, i, cells, local_dofs, local_values):
        """Set x0 for the #`i` unknown at its local positions (indicated by `elements` and `local_dofs`)
        to be `local values`
        """
        self._customizations.append(
            ('set_x0_from_local_dofs', [i, cells, local_dofs, local_values])
        )

    def set_no_evaluation_of_local_dofs(self, i, cells, local_dofs):
        ...
        self._customizations.append(
            ('set_no_evaluation_for_overall_local_dofs', [cells, overall_local_dofs])
        )

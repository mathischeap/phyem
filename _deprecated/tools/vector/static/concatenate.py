...
def concatenate(v_1d_list, gm):
    ...
    for i, vi in enumerate(v_1d_list):
        if vi is None:
            v.append(
                IrregularStaticLocalVector(0, gms[i])
            )
        else:
            assert issubclass(vi.__class__, IrregularStaticLocalVector) and vi._gm is gms[i], f"gm wrong!"
            v.append(
                vi
            )

    cv = _IrregularStaticLocalVectorConcatenate(v)

    return IrregularStaticLocalVector(cv, gm)


class _IrregularStaticLocalVectorConcatenate(Frozen):
    """"""

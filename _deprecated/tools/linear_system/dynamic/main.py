...
from msehy.tools.linear_system.static.local import IrregularStaticLocalLinearSystem


from msehy.tools.linear_system.dynamic.bc import IrregularDynamicLinearSystemBoundaryCondition
from src.wf.mp.linear_system import MatrixProxyLinearSystem

_cs1 = constant_scalar(1)

from msehy.tools.linear_system.dynamic.array_parser import msehy_root_array_parser as ___array_parse___


class IrregularDynamicLinearSystem(Frozen):
    ...
    def _set_bc(self, bc, msepy_base):
        ...
        if bc is None:
            ...
            self._bc = IrregularDynamicLinearSystemBoundaryCondition(
                self, bc, msepy_base
            )
    ...
    def __call__(self, *args, **kwargs):
        ...
        (
            ...
            A_gene_indicating, x_gene_indicating, b_gene_indicating,
            _str_args,
        ...
        return IrregularStaticLocalLinearSystem(
            static_A, static_x, static_b,
            _pr_texts=[A_texts, x_texts, b_texts],
            _time_indicating_text=[A_time_indicating, x_time_indicating, b_time_indicating],
            _gene_indicating_text=[A_gene_indicating, x_gene_indicating, b_gene_indicating],
            _str_args=_str_args,
        )
    ...
    def _get_raw_Axb(self, *args, **kwargs):
        ...
        A_gene_indicating = [[[] for _ in range(num_cols)] for _ in range(num_rows)]
        # do not change '' into something else
        for i in range(num_rows):
            for j in range(num_cols):
                ...
                if Aij is None:
                    ...
                    static_Aij, text, time_indicating, gene_indicating = Aij(*args, **kwargs)
                    ...
                    A_gene_indicating[i][j] = gene_indicating
        ...
        x_gene_indicating = [[] for _ in range(num_cols)]  # do not change '' into something else
        for j in range(num_cols):

            x_j = self._x[j]  # x_j cannot be None

            static_x_j, text, time_indicating, gene_indicating = x_j(*args, **kwargs)

            assert static_x_j.__class__ is IrregularStaticCochainVector, \
                f"entry #{j}  of x is not a IrregularStaticCochainVector!"
            ...
            x_gene_indicating[j] = gene_indicating
        ...
        b_gene_indicating = [[] for _ in range(num_rows)]  # do not change '' into something else
        for i in range(num_rows):
            ...
            if b_i is None:
                ...
                static_b_i, text, time_indicating, gene_indicating = b_i(*args, **kwargs)
                ...
                b_gene_indicating[i] = gene_indicating

        # ----- now we pre-define a static ls to check everything is ok and also to retrieve the gms.
        predefined_sls = IrregularStaticLocalLinearSystem(static_A, static_x, static_b)
        ...
        for i in range(num_rows):
            for j in range(num_cols):
                ...
                if Aij is None:
                    ...
                    static_A[i][j] = IrregularStaticLocalMatrix(
            ...
            if static_b[i] is None:
                # noinspection PyTypeChecker
                static_b[i] = IrregularStaticLocalVector(0, row_gms[i])
            else:
                pass
    ...
    def _parse_matrix_block(self, A):
        """"""
        Si, Sj = A._shape
        rA: List = [[None for _ in range(Sj)] for _ in range(Si)]  # A for this raw ls.
        for i, j in A:
            ...
            if Aij == ([], []):
                ...
                for aij, sign in zip(*Aij):
                    ...
                    raw_terms_ij.append(
                        IrregularDynamicTerm(self, sign, factor, components)
                    )

                rA[i][j] = IrregularDynamicBlockEntry(raw_terms_ij)
    ...
    def _parse_vector_block(self, b):
        """"""
        s = b._shape
        rb: List = [None for _ in range(s)]  # raw b
        for i in range(s):
            bi = b(i)
            if bi == ([], []):
                pass
                ...
                rb[i] = IrregularDynamicBlockEntry(raw_terms_i)
    ...
    def pr(self, figsize=(10, 4)):
        ...
        if _setting['pr_cache']:
            _pr_cache(fig, filename='irregular_DynamicLinearSystem')
        else:
            plt.tight_layout()
...
class IrregularDynamicBlockEntry(Frozen):
    """A bunch of dynamic terms."""

    def __init__(self, raw_terms):
        for rt in raw_terms:
            assert rt.__class__ is IrregularDynamicTerm
    ...
    def __call__(self, *args, **kwargs):
        ...
        for i in self:
            ...
            generation_indicating_i = list()
            indicators = dynamic_term.generation_indicators
            for indicator in indicators:
                assert callable(indicator), f"generation caller must be callable!"
                gen = indicator(*args, **kwargs)
                if isinstance(gen, list):
                    for g in gen:
                        assert g % 1 == 0 and g >= 0, f"generation must be >= 0 integer."
                    generation_indicating_i.append(gen)
                else:
                    assert gen % 1 == 0 and gen >= 0, f"generation must be >= 0 integer."
                    generation_indicating_i.append(int(gen))
            ...
            if hasattr(term, '_gm0_row'):  # must be a static Local matrix
                row_shapes = term._gm0_row._local_shape
                col_shapes = term._gm1_col._local_shape
                _local_text = {}
                num = 0
                for rs, cs in zip(row_shapes, col_shapes):
                    local_shape = (rs, cs)

                    _local_text[num] = str_sign + factor_str + str(local_shape)
                    num += 1

                texts_list.append(_local_text)

            elif hasattr(term, '_gm'):  # must be a static Local vector
                shapes = term._gm._local_shape
                _local_text = {}
                for num, sp in enumerate(shapes):
                    _local_text[num] = str_sign + factor_str + r'\left[' + str(sp) + r'\right]'
                texts_list.append(_local_text)
            ...
            generation_indicating.extend(generation_indicating_i)
        ...
        assert static.__class__ in (
            IrregularStaticLocalMatrix,
            IrregularStaticCochainVector,
            IrregularStaticLocalVector,
        ), f"static={static} is wrong."
        time_indicating = r'+'.join(time_indicating)

        return static, texts_list, time_indicating, generation_indicating


class IrregularDynamicTerm(Frozen):
    ...
    def __init__(self, dls, sign, factor, components):
        ...
        _components = list()
        _mat_sym_repr = r""
        time_indicators: List = list()
        generation_indicators: List = list()
        for comp_lin_repr in components:

            Mat, sym_repr, time_indicator, generation_indicator = ___array_parse___(dls, comp_lin_repr)

            if Mat.__class__ is IrregularStaticLocalMatrix:
                Mat = IrregularDynamicLocalMatrix(Mat)
            else:
                pass

            assert Mat.__class__ in (
                IrregularDynamicLocalMatrix,
                IrregularDynamicCochainVector,
                IrregularDynamicLocalVector,
            ...
            assert callable(generation_indicator), \
                f"generation = {generation_indicator} must be callable."

            time_indicators.append(time_indicator)  # It does not cover time indicator of factor!
            generation_indicators.append(generation_indicator)

        self._time_indicators = time_indicators  # It does not cover time indicator of factor!
        self._generation_indicators = generation_indicators  # It does not cover generation indicator of factor!
    ...
    def time_indicators(self):
        """Does not cover time indicator of `self.factor`."""
        return self._time_indicators

    @property
    def generation_indicators(self):
        """Does not cover generation indicator of `self.factor`."""
        return self._generation_indicators

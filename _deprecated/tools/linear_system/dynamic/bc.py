...
class IrregularDynamicLinearSystemBoundaryCondition(Frozen):
    """"""
    ...
    def config(self, what):
        ...
        if isinstance(abstract_manifold, Manifold):
...
class MsePyDLSEssentialBoundaryCondition(MsePyDLSBoundaryCondition):
    ...
    def config(self, *args):
        ...
        # ---------------- 0: general vc -------------------------------
        if len(args) == 1 and hasattr(args[0], '_is_cf') and args[0]._is_cf():
    ...
    def _find_dofs_and_cochains(self, unknowns, global_or_local='global'):
        ...
        if hasattr(unknown, '_is_form_static_copy') and unknown._is_form_static_copy():
            time = unknown._t
        ...
        elif unknown.__class__ is IrregularStaticCochainVector:
            time = unknown._time
        else:
            raise Exception()

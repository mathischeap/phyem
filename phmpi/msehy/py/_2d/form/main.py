...
from phmpi.msehy.py._2d.form.numeric.main import MPI_MseHy_Py2_Form_Numeric
...
class MPI_MseHy_Py2_Form(Frozen):
    ...
    def __init__(self, abstract_root_form):
        ...
        assert self.space.__class__ is MPI_MseHy_Py2_Space, \
            f"space {self.space} must be a {MPI_MseHy_Py2_Space}."
        ...
        assert self.mesh.generation == 0, \
            f'When initialize msehy-py2 form, the msehy-py mesh must be on G0'
    ...
    def _update(self):
        ...
        if self._do_initialize:
            ...
            if self.mesh.generation == self.generation:
                ...
                assert self.space.generation == self.generation + 1, \
                    f"space generation must only be 1-gen ahead."

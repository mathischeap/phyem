...
class MPI_PY_Localize_Dynamic_Matrix(Frozen):
    """"""

    def __init__(self, data):
        """"""
        if data.__class__ is MPI_PY_Localize_Static_Matrix:
            self._dtype = 'static'
            ...
            raise NotImplementedError(f"MPI_PY2_Localize_Dynamic_Matrix cannot take {data}.")
    ...
    def __call__(self, *args, **kwargs):
        ...
        assert isinstance(static, MPI_PY_Localize_Static_Matrix), f"must return a static one!"

        return static

    def __matmul__(self, other):
        """self @ other"""
        if other.__class__ is MPI_PY_Localize_Dynamic_Matrix:

            mat_list = [self, other]

            return MPI_PY_Localize_Dynamic_Matrix(mat_list)

        elif other.__class__ in (MPI_PY_Localize_Dynamic_Vector, MPI_PY_Localize_Dynamic_Vector_Cochain):

            _matmul = _MPI_PY_MatMul_Vector(self, other)

            return MPI_PY_Localize_Dynamic_Vector(_matmul)

        else:
            raise NotImplementedError(f"{self.__class__.__name__} cannot @ {other.__class__.__name__}")


class _MPI_PY_MatMul_Vector(Frozen):
    """"""

...
def concatenate(v_1d_list, gm):
    ...
    for i, vi in enumerate(v_1d_list):
        if vi is None:
            v.append(
                MPI_PY_Localize_Static_Vector(0, gms[i])
            )
        else:
            assert vi.__class__ in (MPI_PY_Localize_Static_Vector, MPI_PY_Localize_Static_Vector_Cochain)
    ...
    return MPI_PY_Localize_Static_Vector(cv, gm)

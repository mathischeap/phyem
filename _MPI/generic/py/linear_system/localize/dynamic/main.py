...
from _MPI.generic.py.matrix.localize.dynamic import MPI_PY_Localize_Dynamic_Matrix
from _MPI.generic.py.vector.localize.dynamic import (MPI_PY_Localize_Dynamic_Vector,
                                                     MPI_PY_Localize_Dynamic_Vector_Cochain)

from _MPI.generic.py.linear_system.localize.static.main import MPI_PY_Localize_Static_Linear_System

from _MPI.generic.py.linear_system.localize.dynamic.array_parser import MPI_py_array_parser
from _MPI.generic.py.linear_system.localize.dynamic.bc import MPI_PY_Dynamic_Linear_System_BC

_cs1 = constant_scalar(1)


class MPI_PY_Dynamic_Linear_System(Frozen):
    """We study an abstract `mp_ls` and obtain a raw mse-ls which will future be classified into something else."""

    def __init__(self, mp_ls, base):
        """"""
    ...
    def _parse_bc(self, bc, base):
        ...
        if bc is None:
            ...
            self._bc = MPI_PY_Dynamic_Linear_System_BC(
                self, bc, base
            )
    ...
    def apply(self):
        """"""
        self._parse_bc(self._mp_ls._bc, self._base)
        self._A = self._parse_matrix_block(self._mp_ls._ls.A)
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self._bc is None or len(self._bc) == 0:
            ...
            for boundary_section in self._bc:
                ...
                for j, bc in enumerate(bcs):
                    ...
                    assert number_application == 1, f"#{j}th bc={bc} is not handled or over-handled."
        ...
        return MPI_PY_Localize_Static_Linear_System(
            static_A, static_x, static_b,
    ...
    def _get_raw_Axb(self, *args, **kwargs):
        ...
        static_A: list = [[None for _ in range(num_cols)] for _ in range(num_rows)]
        ...
        for j in range(num_cols):
            ...
            assert static_x_j.__class__ is MPI_PY_Localize_Static_Vector_Cochain, \
        ...
        static_b: list = [None for _ in range(num_rows)]
        ...
        predefined_sls = MPI_PY_Localize_Static_Linear_System(static_A, static_x, static_b)
        ...
        for i in range(num_rows):
            for j in range(num_cols):
                Aij = static_A[i][j]
                if Aij is None:
                    static_A[i][j] = MPI_PY_Localize_Static_Matrix(
            ...
            if static_b[i] is None:
                static_b[i] = MPI_PY_Localize_Static_Vector(0, row_gms[i])
            else:
                pass
    ...
    def pr(self, figsize=(10, 4)):
        ...
        if _setting['pr_cache']:
            _pr_cache(fig, filename='mpi_py_msehy_Dynamic_Linear_System')
        else:
            plt.tight_layout()
...
class DynamicBlockEntry(Frozen):
    ...
    def __call__(self, *args, **kwargs):
        ...
        for i in self:
            ...
            if hasattr(term, '_gm_row'):  # must be a MsePy Local matrix
                gm_row = term._gm_row
                gm_col = term._gm_col
                local_shape = (0, 0)
                for index in gm_row:
                ...
                local_shape = 0
                for index in term._gm:
                    local_shape = term._gm.num_local_dofs(index)
                    break
                texts_list.append(
...
class DynamicTerm(Frozen):
    ...
    def __init__(self, dls, sign, factor, components):
        ...
        for comp_lin_repr in components:
            ...
            Mat, sym_repr, time_indicator = MPI_py_array_parser(dls, comp_lin_repr)

            if Mat.__class__ is MPI_PY_Localize_Static_Matrix:
                Mat = MPI_PY_Localize_Dynamic_Matrix(Mat)
            else:
                pass

            assert Mat.__class__ in (
                MPI_PY_Localize_Dynamic_Matrix,
                MPI_PY_Localize_Dynamic_Vector_Cochain,
                MPI_PY_Localize_Dynamic_Vector,
